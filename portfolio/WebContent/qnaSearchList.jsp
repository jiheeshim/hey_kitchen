...
<body>
	<div>
		...
		<main>
			...
			<c:set var="qnaSearchTxt" value='<%= request.getParameter("qnaSearchTxt") %>' />
			<section class="pages">
				<c:if test="${pageInfo.page <= 1}">
					<button>&lt;</button>
				</c:if>
				<c:if test="${pageInfo.page > 1}">
					<button onclick="location.href='qnaSearch.qna?page=${pageInfo.page - 1}&field=${field}&qnaSearchTxt=${qnaSearchTxt}'">&lt;</button>
				</c:if>
				<c:forEach var="p" begin="${pageInfo.startPage}" end="${pageInfo.endPage}">
					...
					<c:if test="${p != pageInfo.page}">
						<a href="qnaSearch.qna?page=${p}&field=${field}&qnaSearchTxt=${qnaSearchTxt}">${p}</a>&nbsp;
					</c:if>
				</c:forEach>
			...
			<section class="qnaSearch">
				<form action="qnaSearch.qna">
					<select name="field">
						...
						<option value="qnaCategory" <c:if test="${field eq 'qnaCategory'}">selected</c:if> >문의분류</option>
						<option value="id" <c:if test="${field eq 'id'}">selected</c:if> >작성자</option>
					...
					<input type="text" name="qnaSearchTxt" value="${qnaSearchTxt}" size="15">
